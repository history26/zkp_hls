...
void processor_1(ep_stream* ep_1,s_stream* s1,ep_stream* res1)
{
#pragma HLS ALLOCATION function instances=UniPadd limit=1
	epoint bucket1[BUCKET_NUM];
#pragma HLS DEPENDENCE dependent=false type=inter variable=bucket1
...
#pragma HLS PIPELINE II=3
	...
	processor_1_label0:for(int ii=0;ii<10;ii++)
	{
...
void processor_2(ep_stream* ep_2,s_stream* s2,ep_stream* res2)
{
#pragma HLS ALLOCATION function instances=UniPadd limit=1
	epoint bucket2[BUCKET_NUM];
#pragma HLS DEPENDENCE dependent=false type=inter variable=bucket2
	ap_uint<2> flags2[BUCKET_NUM/2];
	ep_stream conf_p;
	s_stream conf_s;
	//init warning!!!!!!!
	for(uint32 i=0;i<S_PART;i++)
	{
#pragma HLS PIPELINE II=3
		epoint ptemp=ep_2->read();
		uint16 index=s2->read();
		if(index==0)
			continue;
		if(index[15]==1)
		{
		...
		else if(flags2[indext]==1)
		{
			flags2[indext][1]=1;
			bucket2[indext+32768]=UniPadd(ptemp,bucket2[indext+32768],bls377_k,bls377_q,bls377_i);
			flags2[indext][1]=0;
		}
		else
		{
			flags2[indext][0]=1;
			bucket2[indext]=UniPadd(ptemp,bucket2[indext],bls377_k,bls377_q,bls377_i);
			flags2[indext][0]=0;
		}
	}
...
#pragma HLS PIPELINE off
		...
		epoint ptemp=conf_p.read();
		uint16 index=conf_s.read();
		bucket2[index-1]=UniPadd(ptemp,bucket2[index-1],bls377_k,bls377_q,bls377_i);
	}
	for(int j=0;j<32768;j++)
	{
#pragma HLS PIPELINE II=3
		bucket2[j]=UniPadd(bucket2[j],bucket2[j+32768],bls377_k,bls377_q,bls377_i);
	}
	epoint tmp[32];
#pragma HLS DEPENDENCE dependent=false type=inter variable=tmp
	epoint tmp1[32];
...
#pragma HLS PIPELINE II=3
		...
		if(k[5]==0)
		{
			tmp[ind]=UniPadd(tmp[ind],bucket2[ind1],bls377_k,bls377_q,bls377_i);
		}
		else
		{
			tmp1[ind]=UniPadd(tmp1[ind],tmp[ind],bls377_k,bls377_q,bls377_i);
...
void processor_3(ep_stream* ep_3,s_stream* s3,ep_stream* res3)
{
#pragma HLS ALLOCATION function instances=UniPadd limit=1
	epoint bucket3[BUCKET_NUM];
#pragma HLS DEPENDENCE dependent=false type=inter variable=bucket3
	ap_uint<2> flags3[BUCKET_NUM/2];
	ep_stream conf_p;
	s_stream conf_s;
	//init warning!!!!!!!
	for(uint32 i=0;i<S_PART;i++)
	{
#pragma HLS PIPELINE II=3
		epoint ptemp=ep_3->read();
		uint16 index=s3->read();
		if(index==0)
			continue;
		if(index[15]==1)
		{
		...
		else if(flags3[indext]==1)
		{
			flags3[indext][1]=1;
			bucket3[indext+32768]=UniPadd(ptemp,bucket3[indext+32768],bls377_k,bls377_q,bls377_i);
			flags3[indext][1]=0;
		}
		else
		{
			flags3[indext][0]=1;
			bucket3[indext]=UniPadd(ptemp,bucket3[indext],bls377_k,bls377_q,bls377_i);
			flags3[indext][0]=0;
		}
	}
	for(int ii=0;ii<10;ii++)
...
#pragma HLS PIPELINE off
		...
		epoint ptemp=conf_p.read();
		uint16 index=conf_s.read();
		bucket3[index-1]=UniPadd(ptemp,bucket3[index-1],bls377_k,bls377_q,bls377_i);
	}
	for(int j=0;j<32768;j++)
	{
#pragma HLS PIPELINE II=3
		bucket3[j]=UniPadd(bucket3[j],bucket3[j+32768],bls377_k,bls377_q,bls377_i);
	}
	epoint tmp[32];
#pragma HLS DEPENDENCE dependent=false type=inter variable=tmp
	epoint tmp1[32];
...
#pragma HLS PIPELINE II=3
		...
		if(k[5]==0)
		{
			tmp[ind]=UniPadd(tmp[ind],bucket3[ind1],bls377_k,bls377_q,bls377_i);
		}
		else
		{
			tmp1[ind]=UniPadd(tmp1[ind],tmp[ind],bls377_k,bls377_q,bls377_i);
...
void write_ep(ep_stream* ep_1,ep_stream* ep_2,ep_stream* ep_3,epoint* res)
{
	epoint t1=ep_1->read();
	res[0]=t1;
	epoint t2=ep_2->read();
...
#pragma HLS ALLOCATION function instances=UniPadd limit=3
...
#pragma HLS DATAFLOW
	ep_stream ep_s1,ep_s2,ep_s3,ep_res1,ep_res2,ep_res3;
	s_stream index_s0,index_s1,index_s2,index_s3;
	epoint res1,res2,res3;
	read_ep(p_1,p_2,p_3,p_4,p_5,p_6,&ep_s1,&ep_s2,&ep_s3);
	read_s(scalars,&index_s0);
	divide_s(&index_s0,&index_s1,&index_s2,&index_s3);
	processor_1(&ep_s1,&index_s1,&ep_res1);
	processor_2(&ep_s2,&index_s2,&ep_res2);
	processor_3(&ep_s3,&index_s3,&ep_res3);
	write_ep(&ep_res1,&ep_res2,&ep_res3,res);
